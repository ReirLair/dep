...
app.get('/create-space', async (req, res) => {
    const username = 'vpyserver'; // Update with Hugging Face username
...
RUN mkdir -p /root/.local /root/.cache/pip && chmod -R 777 /root
...
RUN mkdir -p /root/.npm-global /root/.npm-global/.cache /root/.config && chmod -R 777 /root
...
`;
    ...
    try {
        ...
        res.json({
            success: true,
            message: 'Hugging Face Space Created Successfully',
            spaceURL,
            owner: 'Reiker'
        });
